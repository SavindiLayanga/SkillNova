      ...
      {isTextarea ? (
        <textarea
          ...
          className={clsx(
            ...
            isMissing ? "border-rose-300 focus:ring-rose-500/30 bg-rose-50 placeholder-rose-400 text-rose-900" : "border-slate-200 focus:ring-primary-500/30 focus:border-primary-500 bg-white text-slate-900 placeholder-slate-400"
          )}
        />
      ) : (
        <input
...
const TagInput = ({ label, tags = [], onChange, placeholder = "Type and press enter..." }) => {
  ...
  return (
    <div className="flex flex-col gap-1.5 w-full">
      ...
      <div className="min-h-[42px] w-full px-2 py-1.5 border border-slate-200 focus-within:ring-2 focus-within:ring-primary-500/30 focus-within:border-primary-500 rounded-lg bg-white flex flex-wrap gap-2 items-center transition-all">
        {safeTags.map((tag, index) => {
          const safeTagStr = typeof tag === "object" ? JSON.stringify(tag) : String(tag);
          return (
            <span key={index} className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-xs font-medium bg-primary-50 text-primary-700 border border-primary-200">
              {safeTagStr}
    ...
    <div className="w-full space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="mb-8">
        <h2 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
          <Edit2 className="w-6 h-6 text-primary-600" /> Verify Your Information
        </h2>
      ...
      <Card className="p-6 border border-slate-200 shadow-sm bg-white">
        <h3 className="text-lg font-bold text-slate-900 border-b border-slate-100 pb-3 mb-5">Personal Information</h3>
      ...
      <Card className="p-6 border border-slate-200 shadow-sm bg-white">
        ...
        <div className="space-y-6">
          {(!Array.isArray(formData.experience) || formData.experience.length === 0) && <p className="text-sm text-slate-500 italic">No experience added.</p>}
          {Array.isArray(formData.experience) && formData.experience.map((exp, idx) => (
            <div key={idx} className="p-4 rounded-xl border border-slate-200 bg-slate-50 relative group">
              <button onClick={() => removeArrayItem("experience", idx)} className="absolute top-4 right-4 text-slate-400 hover:text-rose-500 opacity-0 group-hover:opacity-100 transition-opacity">
      ...
      <Card className="p-6 border border-slate-200 shadow-sm bg-white">
        ...
        <div className="space-y-6">
          {(!Array.isArray(formData.education) || formData.education.length === 0) && <p className="text-sm text-slate-500 italic">No education added.</p>}
          {Array.isArray(formData.education) && formData.education.map((edu, idx) => (
            <div key={idx} className="p-4 rounded-xl border border-slate-200 bg-slate-50 relative group">
              <button onClick={() => removeArrayItem("education", idx)} className="absolute top-4 right-4 text-slate-400 hover:text-rose-500 opacity-0 group-hover:opacity-100 transition-opacity">
                <Trash2 className="w-4 h-4" />
              </button>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pr-6">
      ...
      <Card className="p-6 border border-slate-200 shadow-sm bg-white">
        <h3 className="text-lg font-bold text-slate-900 border-b border-slate-100 pb-3 mb-5">Skills & Expertise</h3>
      ...
      <div className="flex flex-col sm:flex-row gap-4 pt-4 pb-12">
        ...
        <Button 
          variant="secondary"
          className="w-full sm:w-auto justify-center bg-white border-slate-200 text-slate-700 hover:bg-slate-50 py-3" 
          icon={RotateCcw}
          onClick={onCancel}
        >
          Cancel
